...
function MyCourses (courseName,courseImgSrc,instuctorName,instuctorImg,courseHours,coursePrise)
{
    this.courseName=courseName;
    this.courseImgSrc=courseImgSrc;
    this.instuctorName=instuctorName;
    this.instuctorImg=instuctorImg;
...
formEl.addEventListener('submit', addCourse);
 ...
 function addCourse(event){
    ...
    ++counter;
    let instuctorName = event.target.children[1].children[0].children[1].innerText;

    let courseName= event.target.children[0].children[2].innerText;

    let courseImgSrc= event.target.children[0].children[0].src;

    let instuctorImg = event.target.children[1].children[0].children[0].src;
...
let newCourse = new MyCourses (courseName,courseImgSrc,instuctorName,instuctorImg,courseHours,coursePrise );
settingCourses();
settingCounter();
